...
def _latex_poly_equation(coef: np.ndarray, expr: str, logy: bool) -> str:
    ...
    """
    if coef is None or len(coef) == 0:
        return r"\begin{equation}\end{equation}\begin{equation}\end{equation}"
    d = len(coef) - 1
    parts = []
    for i, c in enumerate(coef):
        p = d - i
...
def _latex_piecewise_inline(b: float, k1: float, dk: float, xb: float, expr: str) -> str:
    ...
    \[
      ...
      \end{cases}
    \]
    with p_2=p_1+\Delta kp_2=p_1+\Delta k and continuity \(A_2=A_1 (r_b/\text{AU})^{p_1-p_2}\).
    """
    A1   = float(np.exp(b))
    p1   = float(k1)
    p2   = float(k1 + dk)
...
def _blocked_cv_lppd_fgls(
    ...
    *,
    ...
    For test fold with design V_{\text{te}}V_{\text{te}} and variance model \(\ln\sigma^2= q(x)\):
    \[
