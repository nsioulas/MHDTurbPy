...
def estimate_local_wavelet_geometry(
    ...
    Bmag_raw = func.estimate_vec_magnitude(B_comp.to_numpy(dtype=float)).reshape(-1, 1)
    ...
    dBmod_nT_all = details_all[:, :, 10:11]
    ...
    for j in range(n_levels):
        ...
        keep_turb_amp = {
            ...
            'dBmod_nT': _mask_array(dBmod_nT_all[j, :, 0], mask),
            'B_l': B_l,
        }
...
def _scalar_energy_fraction_from_flucts(dscalar: np.ndarray, dvec: np.ndarray) -> np.ndarray:
    num = np.asarray(dscalar, dtype=float) ** 2
    den = np.nansum(np.asarray(dvec, dtype=float) ** 2, axis=-1)
    with np.errstate(divide='ignore', invalid='ignore'):
        return num / den
...
_DEFAULT_COMPACT_COEFF_COLUMNS = (
    ...
    'W_B_mag', 'B_perp',
    'compress_simple', 'compress_simple_parallel', 'compress_simple_absB',
)
...
def _build_minimal_compact_level_columns(
    ...
    """Build the lean row-level coefficient payload for one level.

    The first pass should save only what is needed for the second-pass conditional
    analysis. The default export therefore contains only:

    * local scales used later for bucket-specific reductions;
    * local angles used later to reconstruct directional buckets;
    * exact normalization factors for PSDs and scale-normalized moments;
    * a small set of row-level conditioning variables;
    * the magnetic trace magnitude, the magnetic perpendicular magnitude, and the core magnetic compressibility ratios.

    Projected components and other diagnostics are optional. They are saved only when
    they are requested explicitly through ``ts_list``. This keeps the first-pass store
    compact and avoids computing basis projections that are not needed.
    """
    del theta_mask_perp, theta_mask_disp, theta_mask_par, theta_mask_par_rest  # bucket masks are reconstructed later from angles

    n_rows = int(level_data.l_mag.shape[0])
    if n_rows == 0:
        return {key: np.empty((0,), dtype=np.float64) for key in _DEFAULT_COMPACT_COEFF_COLUMNS}

    requested = set(_explicit_requested_keys(ts_list))
    b_export_units = str(raw_context.get('B_export_units', 'vel'))
    ...
    if b_export_units.lower().startswith('vel'):
        b_trace = np.asarray(level_data.dVa, dtype=np.float64)
        b_perp_mag = np.asarray(_estimate_vec_magnitude_stacked(level_data.dVa_perp), dtype=np.float64)
    else:
        b_trace = np.asarray(level_data.dB_nT, dtype=np.float64)
        b_perp_mag = np.asarray(_estimate_vec_magnitude_stacked(level_data.dB_perp_nT), dtype=np.float64)

    compress_parallel = np.asarray(_parallel_energy_fraction_from_flucts(level_data.dB_parallel_nT, level_data.dB_nT), dtype=np.float64)
    compress_absB = np.asarray(_scalar_energy_fraction_from_flucts(level_data.keep_turb_amp.get('dBmod_nT', np.full(n_rows, np.nan)), level_data.dB_nT), dtype=np.float64)
    ...
    column_data: Dict[str, np.ndarray] = {
        ...
        'W_B_mag': np.asarray(_estimate_vec_magnitude_stacked(b_trace), dtype=np.float64),
        'B_perp': b_perp_mag,
        'compress_simple': compress_parallel,
        'compress_simple_parallel': compress_parallel,
        'compress_simple_absB': compress_absB,
    }

    optional_conditionals = {
        ...
        'compress_simple_V': np.asarray(
            _parallel_energy_fraction_from_flucts(
                _perp_vector_stacked(level_data.dV, level_data.B_l, return_paral_comp=True)[1],
                level_data.dV,
            ),
            dtype=np.float64,
        ),
    }
    for key in list(optional_conditionals):
        if key in requested:
            column_data[key] = optional_conditionals[key]

    # Optional metadata. Keep these out of the default export because they are not
    # required for the standard conditional PSD / moment reduction.
    optional_metadata = {
        ...
        'tau_equiv_samples': np.full(n_rows, float(level_data.tau_equiv_samples), dtype=np.float64),
        'scale_seconds': np.full(n_rows, float(level_data.scale_seconds), dtype=np.float64),
        'scale_samples': np.full(n_rows, float(level_data.scale_samples), dtype=np.float64),
        'period_s': np.full(n_rows, float(level_data.period_s), dtype=np.float64),
        'bandwidth_hz': np.full(n_rows, float(level_data.bandwidth_hz), dtype=np.float64),
    }
    for key in list(optional_metadata):
        if key in requested:
            column_data[key] = optional_metadata[key]

    requested_projection_keys = {
        'B_par', 'B_xi', 'B_lambda',
        'V_par', 'V_perp', 'V_xi', 'V_lambda',
        'Zp_par', 'Zp_perp', 'Zp_xi', 'Zp_lambda',
        'Zm_par', 'Zm_perp', 'Zm_xi', 'Zm_lambda',
        # legacy aliases kept only when requested explicitly
        'B_nT_par', 'B_nT_perp', 'B_vel_par', 'B_vel_perp',
        'W_B_nT_mag', 'W_B_vel_mag', 'W_V_mag', 'W_Zp_mag', 'W_Zm_mag',
    }
    need_projections = bool(requested_projection_keys & requested)
    need_basis = bool({
        'B_par', 'B_xi', 'B_lambda',
        'V_par', 'V_xi', 'V_lambda',
        'Zp_par', 'Zp_xi', 'Zp_lambda',
        'Zm_par', 'Zm_xi', 'Zm_lambda',
        'B_nT_par', 'B_vel_par',
    } & requested)

    e_l = e_xi = e_lambda = None
    if need_basis:
        e_l, e_xi, e_lambda, _basis_valid = _local_basis_from_background_and_perp(level_data.B_l, level_data.dB_perp_nT)

    if need_projections:
        def _add_if_requested(key: str, value: np.ndarray) -> None:
            if key in requested:
                column_data[key] = np.asarray(value, dtype=np.float64)

        # Export B projections in the user-selected units only.
        if b_export_units.lower().startswith('vel'):
            _add_if_requested('W_B_vel_mag', _estimate_vec_magnitude_stacked(level_data.dVa))
            _add_if_requested('B_vel_par', _abs_scalar_projection(level_data.dVa, e_l) if e_l is not None else np.full(n_rows, np.nan))
            _add_if_requested('B_vel_perp', _estimate_vec_magnitude_stacked(level_data.dVa_perp))
            b_proj = np.asarray(level_data.dVa, dtype=np.float64)
        else:
            _add_if_requested('W_B_nT_mag', _estimate_vec_magnitude_stacked(level_data.dB_nT))
            _add_if_requested('B_nT_par', _abs_scalar_projection(level_data.dB_nT, e_l) if e_l is not None else np.full(n_rows, np.nan))
            _add_if_requested('B_nT_perp', _estimate_vec_magnitude_stacked(level_data.dB_perp_nT))
            b_proj = np.asarray(level_data.dB_nT, dtype=np.float64)

        if 'B_par' in requested:
            column_data['B_par'] = np.asarray(_abs_scalar_projection(b_proj, e_l), dtype=np.float64) if e_l is not None else np.full(n_rows, np.nan)
        if 'B_perp' in requested:
            column_data['B_perp'] = b_perp_mag
        if 'B_xi' in requested:
            column_data['B_xi'] = np.asarray(_abs_scalar_projection(b_proj, e_xi), dtype=np.float64) if e_xi is not None else np.full(n_rows, np.nan)
        if 'B_lambda' in requested:
            column_data['B_lambda'] = np.asarray(_abs_scalar_projection(b_proj, e_lambda), dtype=np.float64) if e_lambda is not None else np.full(n_rows, np.nan)

        _add_if_requested('W_V_mag', _estimate_vec_magnitude_stacked(level_data.dV))
        _add_if_requested('W_Zp_mag', _estimate_vec_magnitude_stacked(level_data.dZp))
        _add_if_requested('W_Zm_mag', _estimate_vec_magnitude_stacked(level_data.dZm))

        if {'V_par', 'V_perp', 'V_xi', 'V_lambda'} & requested:
            v_perp_vec = _perp_vector_stacked(level_data.dV, level_data.B_l)
            _add_if_requested('V_par', _abs_scalar_projection(level_data.dV, e_l) if e_l is not None else np.full(n_rows, np.nan))
            _add_if_requested('V_perp', _estimate_vec_magnitude_stacked(v_perp_vec))
            _add_if_requested('V_xi', _abs_scalar_projection(level_data.dV, e_xi) if e_xi is not None else np.full(n_rows, np.nan))
            _add_if_requested('V_lambda', _abs_scalar_projection(level_data.dV, e_lambda) if e_lambda is not None else np.full(n_rows, np.nan))

        if {'Zp_par', 'Zp_perp', 'Zp_xi', 'Zp_lambda'} & requested:
            zp_perp_vec = _perp_vector_stacked(level_data.dZp, level_data.B_l)
            _add_if_requested('Zp_par', _abs_scalar_projection(level_data.dZp, e_l) if e_l is not None else np.full(n_rows, np.nan))
            _add_if_requested('Zp_perp', _estimate_vec_magnitude_stacked(zp_perp_vec))
            _add_if_requested('Zp_xi', _abs_scalar_projection(level_data.dZp, e_xi) if e_xi is not None else np.full(n_rows, np.nan))
            _add_if_requested('Zp_lambda', _abs_scalar_projection(level_data.dZp, e_lambda) if e_lambda is not None else np.full(n_rows, np.nan))

        if {'Zm_par', 'Zm_perp', 'Zm_xi', 'Zm_lambda'} & requested:
            zm_perp_vec = _perp_vector_stacked(level_data.dZm, level_data.B_l)
            _add_if_requested('Zm_par', _abs_scalar_projection(level_data.dZm, e_l) if e_l is not None else np.full(n_rows, np.nan))
            _add_if_requested('Zm_perp', _estimate_vec_magnitude_stacked(zm_perp_vec))
            _add_if_requested('Zm_xi', _abs_scalar_projection(level_data.dZm, e_xi) if e_xi is not None else np.full(n_rows, np.nan))
            _add_if_requested('Zm_lambda', _abs_scalar_projection(level_data.dZm, e_lambda) if e_lambda is not None else np.full(n_rows, np.nan))

    # Any remaining explicit requests are delegated to the existing quantity builder.
    manual_keys = set(column_data)
    remaining_extra_keys = [key for key in _explicit_requested_keys(ts_list) if key not in manual_keys]
    if remaining_extra_keys:
        extra_payload = estimate_requested_quantities(level_data, raw_context, ts_list=remaining_extra_keys)
        for key in remaining_extra_keys:
