...
def filter_gaps_by_min_duration(gaps: pd.DataFrame, min_gap: str | pd.Timedelta) -> pd.DataFrame:
    """
...
def merge_gap_intervals(gaps: pd.DataFrame, merge_tol: str | pd.Timedelta = "0s") -> pd.DataFrame:
    """
...
def mask_df_with_gaps(df: pd.DataFrame, gaps: pd.DataFrame, columns=None) -> pd.DataFrame:
    """
