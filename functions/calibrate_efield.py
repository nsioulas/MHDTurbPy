"""
psp_efield_calibration.py
=========================
Robust pipeline for PSP DC Electric Field Calibration.
Uses signal_processing.synchronize_dfs for alignment.
"""
...
try:
    ...
    import signal_processing as func

# ==========================================
# 1. PHYSICS & FITTING
# ==========================================

def coupled_residuals(params, E_conv_x, E_conv_y, E_meas_x, E_meas_y):
    """
    Robust Cost Function for Coupled Linear Model.
    """
    a, b, c, d = params
    pred_x = a * E_conv_x + b * E_conv_y + c
    pred_y = a * E_conv_y - b * E_conv_x + d
    return np.concatenate([pred_x - E_meas_x, pred_y - E_meas_y])

def fit_window(subset, min_points=10):
    """
    Fits one window using Robust Least Squares (Soft-L1).
    """
    if len(subset) < min_points:
        return np.full(4, np.nan), np.nan

    # Units: V (km/s -> m/s), B (nT -> T)
    Vx = subset['Vx'].values * 1e3
    Vy = subset['Vy'].values * 1e3
    Vz = subset['Vz'].values * 1e3
    
    Bx = subset['Bx'].values * 1e-9
    By = subset['By'].values * 1e-9
    Bz = subset['Bz'].values * 1e-9
    
    # Convective Field: - (V x B)
    Ec_x = -(Vy*Bz - Vz*By) 
    Ec_y = -(Vz*Bx - Vx*Bz)
    
    # Measured Data
    Em_x = subset['dvx'].values
    Em_y = subset['dvy'].values
    
    try:
        res = least_squares(
            coupled_residuals, 
            [1.0, 0.0, 0.0, 0.0], 
            args=(Ec_x, Ec_y, Em_x, Em_y),
            loss='soft_l1', 
            f_scale=0.1 
        )
        quality = np.mean(np.abs(res.fun))
        return res.x, quality
    except Exception:
        return np.full(4, np.nan), np.nan
...
def calibrate_electric_field_pipeline(bdf, vdf, edf, 
                                      window_min=20, 
                                      stride_min=5, 
                                      min_bz_for_ez=2.0,
                                      max_ez_physical=500.0,
                                      n_jobs=-1):
    
    print("Step 1: Synchronizing data to Velocity (Low Freq) cadence...")
    
    # --- A. Sync B (Higher) to V (Lower) ---
    # upsample=False -> Returns (High_Downsampled, Low_Original)
    bdf_fit, vdf_fit_b = func.synchronize_dfs(bdf, vdf, upsample=False)
    
    # --- B. Sync E (Higher) to V (Lower) ---
    edf_fit, vdf_fit_e = func.synchronize_dfs(edf, vdf, upsample=False)
    
    # --- C. Intersect to common valid index ---
    common_idx = vdf_fit_b.index.intersection(vdf_fit_e.index).intersection(bdf_fit.index).intersection(edf_fit.index)
    
    if len(common_idx) < 10:
        raise ValueError("Insufficient overlap between V, B, and E dataframes.")

    df_fit = pd.concat([
        vdf_fit_b.loc[common_idx], 
        bdf_fit.loc[common_idx],   
        edf_fit.loc[common_idx]    
    ], axis=1).dropna()
    
    print(f"      Fit Data Ready: {len(df_fit)} points.")

    # --- 2. Sliding Window Fitting ---
    print("Step 2: Running Sliding Window Calibration...")
    
    t_start = df_fit.index[0]
    t_end = df_fit.index[-1]
    window_size = pd.Timedelta(minutes=window_min)
    stride = pd.Timedelta(minutes=stride_min)
    
    windows = []
    current_t = t_start
    while current_t + window_size <= t_end:
        windows.append((current_t, current_t + window_size))
        current_t += stride
        
    def process_one_window(w_start, w_end):
        mask = (df_fit.index >= w_start) & (df_fit.index < w_end)
        if not mask.any(): return None
        coeffs, qual = fit_window(df_fit.loc[mask])
        return {
            'time': w_start + (w_end - w_start)/2,
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(process_one_window)(s, e) for s, e in windows
    )
    
    results = [r for r in results if r is not None]
    if not results:
        raise RuntimeError("Calibration failed: No valid windows found.")
        
    coeff_df = pd.DataFrame(results).set_index('time').sort_index().dropna()

    # --- 3. Apply to High-Cadence Data ---
    print("Step 3: Applying Calibration to High-Cadence E-field...")
    
    # Interpolate coeffs to High-Cadence E index
    coeff_high = coeff_df.reindex(edf.index.union(coeff_df.index)).sort_index()
    coeff_high = coeff_high.interpolate(method='time', limit_direction='both')
    coeff_high = coeff_high.reindex(edf.index)
    
    a, b = coeff_high['a'].values, coeff_high['b'].values
    c, d = coeff_high['c'].values, coeff_high['d'].values
    denom = a**2 + b**2
    denom[denom == 0] = np.nan
    
    dvx, dvy = edf['dvx'].values, edf['dvy'].values
    
    Ex_Vm = (a * (dvx - c) - b * (dvy - d)) / denom
    Ey_Vm = (b * (dvx - c) + a * (dvy - d)) / denom
    
    calibrated_df = pd.DataFrame(index=edf.index)
    calibrated_df['Ex'] = Ex_Vm * 1e3 # mV/m
    calibrated_df['Ey'] = Ey_Vm * 1e3 
    
    # Diagnostics
    calibrated_df['Leff'] = np.sqrt(denom)
    calibrated_df['theta_deg'] = np.degrees(np.arctan2(b, a))

    # --- 4. Ez Reconstruction ---
    print("Step 4: Reconstructing Ez (High Res)...")
    
    # Use synchronize_dfs with upsample=True to bring B up to E cadence
    # Returns (Original High, Upsampled Low) -> (E_original, B_upsampled)
    edf_sync_ez, bdf_upsampled = func.synchronize_dfs(edf, bdf, upsample=True)
    
    # Align calibrated Ex/Ey
    cal_subset = calibrated_df.reindex(edf_sync_ez.index)
    
    # Extract arrays
    Bx = bdf_upsampled['Bx'].values * 1e-9
    By = bdf_upsampled['By'].values * 1e-9
    Bz = bdf_upsampled['Bz'].values * 1e-9
    
    Ex = cal_subset['Ex'].values * 1e-3
    Ey = cal_subset['Ey'].values * 1e-3
    
    # Mask Singularity 
    valid_mask = (np.abs(Bz) > (min_bz_for_ez * 1e-9))
    
    Ez = np.full_like(Ex, np.nan)
    numerator = -(Ex[valid_mask] * Bx[valid_mask] + Ey[valid_mask] * By[valid_mask])
    Ez[valid_mask] = numerator / Bz[valid_mask]
    
    # Filter Physical Spikes
    Ez[np.abs(Ez) > (max_ez_physical / 1e3)] = np.nan
    
    # Assign back
    calibrated_df['Ez'] = np.nan
    calibrated_df.loc[edf_sync_ez.index, 'Ez'] = Ez * 1e3 # mV/m
    
    print("Calibration Complete.")
    return calibrated_df, coeff_df
