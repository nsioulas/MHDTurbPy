...
def build_wavelet_frame(n_samples: int, dt: float, wname: str = 'mw8', level: Optional[int] = None, level_mode: str = 'recommended') -> Dict[str, Any]:
    ...
    # Approximate half-power frequencies from the mother envelope x^M exp(-x^2/2).
    xs = np.logspace(-4, 3, 30000)
    mother = (xs ** spec.order) * np.exp(-0.5 * xs * xs)
    x_peak = xs[int(np.nanargmax(mother))]
    target = 0.5 * float(np.nanmax(mother))
    below = xs[xs < x_peak]
    above = xs[xs > x_peak]
    m_below = mother[xs < x_peak]
    m_above = mother[xs > x_peak]
    ...
    response_energy_integral = np.full(scale_seconds.shape, _wavelet_response_energy_integral(spec.order), dtype=float)
...
def estimate_wavelet_interval(
    ...
    wavelet_moments = {
        'B': _empty_moment_container(),
        'V': _empty_moment_container(),
        'Zp': _empty_moment_container(),
        'Zm': _empty_moment_container(),
        'LeaderB': _empty_moment_container(),
        'counts': {key: np.zeros(n_levels, dtype=int) for key in nbins},
        'l_di': {key: np.full(n_levels, np.nan) for key in nbins},
    }
    sf = {
        'B': _empty_moment_container(),
        'V': _empty_moment_container(),
        ...
        'Zm': _empty_moment_container(),
        'LeaderB': _empty_moment_container(),
        'counts': wavelet_moments['counts'],
        'l_di': wavelet_moments['l_di'],
    }
    spectra = {
        'B': {key: np.full(n_levels, np.nan) for key in nbins},
        'V': {key: np.full(n_levels, np.nan) for key in nbins},
        'Zp': {key: np.full(n_levels, np.nan) for key in nbins},
        'Zm': {key: np.full(n_levels, np.nan) for key in nbins},
        'LeaderB': {key: np.full(n_levels, np.nan) for key in nbins},
        'counts': sf['counts'],
        'l_di': sf['l_di'],
        'frequency_hz': np.full(n_levels, np.nan),
        'bandwidth_hz': np.full(n_levels, np.nan),
        'response_energy_integral': np.full(n_levels, np.nan),
    }
    legacy_spectra = {
        'B': {key: np.full(n_levels, np.nan) for key in nbins},
        'V': {key: np.full(n_levels, np.nan) for key in nbins},
    ...
    for j, lvl in enumerate(results['levels']):
        ...
        for bucket, idx_sel, ell_arr in mapping:
            if idx_sel.size:
                wavelet_moments['B'][bucket][j] = _moments_from_precomputed_powers(power_lookup['B'], idx_sel)
                wavelet_moments['V'][bucket][j] = _moments_from_precomputed_powers(power_lookup['V'], idx_sel)
                wavelet_moments['Zp'][bucket][j] = _moments_from_precomputed_powers(power_lookup['Zp'], idx_sel)
                wavelet_moments['Zm'][bucket][j] = _moments_from_precomputed_powers(power_lookup['Zm'], idx_sel)
                wavelet_moments['LeaderB'][bucket][j] = _moments_from_precomputed_powers(power_lookup['LeaderB'], idx_sel)
                sf['B'][bucket][j] = _increment_equivalent_moments_from_wavelet_moments(wavelet_moments['B'][bucket][j], q, (lvl.tau_equiv_samples * dt))
                sf['V'][bucket][j] = _increment_equivalent_moments_from_wavelet_moments(wavelet_moments['V'][bucket][j], q, (lvl.tau_equiv_samples * dt))
                sf['Zp'][bucket][j] = _increment_equivalent_moments_from_wavelet_moments(wavelet_moments['Zp'][bucket][j], q, (lvl.tau_equiv_samples * dt))
                sf['Zm'][bucket][j] = _increment_equivalent_moments_from_wavelet_moments(wavelet_moments['Zm'][bucket][j], q, (lvl.tau_equiv_samples * dt))
                sf['LeaderB'][bucket][j] = _increment_equivalent_moments_from_wavelet_moments(wavelet_moments['LeaderB'][bucket][j], q, (lvl.tau_equiv_samples * dt))
                sf['l_di'][bucket][j] = np.nanmean(ell_arr[idx_sel])
                if q2_idx.size:
                    q2 = q2_idx[0]
                    spectra['B'][bucket][j] = wavelet_moments['B'][bucket][j, q2] / max(lvl.response_energy_integral, _EPS)
                    spectra['V'][bucket][j] = wavelet_moments['V'][bucket][j, q2] / max(lvl.response_energy_integral, _EPS)
                    spectra['Zp'][bucket][j] = wavelet_moments['Zp'][bucket][j, q2] / max(lvl.response_energy_integral, _EPS)
                    spectra['Zm'][bucket][j] = wavelet_moments['Zm'][bucket][j, q2] / max(lvl.response_energy_integral, _EPS)
                    spectra['LeaderB'][bucket][j] = wavelet_moments['LeaderB'][bucket][j, q2] / max(lvl.response_energy_integral, _EPS)
                    legacy_spectra['B'][bucket][j] = wavelet_moments['B'][bucket][j, q2] / max(lvl.bandwidth_hz, _EPS)
                    legacy_spectra['V'][bucket][j] = wavelet_moments['V'][bucket][j, q2] / max(lvl.bandwidth_hz, _EPS)
                    legacy_spectra['Zp'][bucket][j] = wavelet_moments['Zp'][bucket][j, q2] / max(lvl.bandwidth_hz, _EPS)
                    legacy_spectra['Zm'][bucket][j] = wavelet_moments['Zm'][bucket][j, q2] / max(lvl.bandwidth_hz, _EPS)
                    legacy_spectra['LeaderB'][bucket][j] = wavelet_moments['LeaderB'][bucket][j, q2] / max(lvl.bandwidth_hz, _EPS)
    ...
    wavelet_moments_out = {
        'B': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in wavelet_moments['B'].items()},
        'V': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in wavelet_moments['V'].items()},
        'Zp': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in wavelet_moments['Zp'].items()},
        'Zm': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in wavelet_moments['Zm'].items()},
        'LeaderB': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in wavelet_moments['LeaderB'].items()},
        'counts': wavelet_moments['counts'],
        'l_di': wavelet_moments['l_di'],
    }
    sfuncs = {
        'B': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in sf['B'].items()},
        'V': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in sf['V'].items()},
        'Zp': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in sf['Zp'].items()},
        'Zm': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in sf['Zm'].items()},
        'LeaderB': {k: (v.T if isinstance(v, np.ndarray) and v.ndim == 2 else v) for k, v in sf['LeaderB'].items()},
        'counts': sf['counts'],
        'l_di': sf['l_di'],
    }
...
def run_filterbank_interval_analysis(
    ...
    try:
        ...
        keep_sfuncs_final = {
            ...
            'meta': {
                'analysis_chain': [
                    f'{_PACKAGE_NAME}/notebooks/3D_LOGFILTERBANK_sfuncs.ipynb',
                ...
                'note': 'Sfuncs/StructureFunctions store increment-equivalent wavelet-moment surrogates obtained by removing the universal L2-wavelet tau^(q/2) factor at the same tau_equiv used in Taylor mapping. They are not strict finite-difference structure functions. WaveletMoments stores the raw conditional wavelet-coefficient moments. Spectra stores second-order raw wavelet moments divided by the exact filter-energy integral; LegacySpectraBandwidth preserves the previous bandwidth-based normalization for comparison. The Gaussian background and even-DoG fluctuation are paired filters, not an exact additive reconstruction. When spacecraft-corrected velocity is supplied, both the Taylor mapping and the velocity/Elsasser fluctuations use that corrected velocity consistently. Raw contextual diagnostics are evaluated on aligned raw series rather than on the gap-filled transform inputs. If the frame does not identify a radial-like component, polarity correction falls back to unity rather than guessing from the first component. Magnetic fluctuation exports use the user-selected unit system.',
