...
def load_and_merge_coefficient_stores(paths: Union[str, Sequence[str]], bucket: str = 'ell_all') -> pd.DataFrame:
    ...
    for path in _expand_paths(paths):
        ...
        df = coefficient_store_to_dataframe(store, bucket=bucket)
        df['source_file'] = path
        frames.append(df)
    if not frames:
        return pd.DataFrame()
