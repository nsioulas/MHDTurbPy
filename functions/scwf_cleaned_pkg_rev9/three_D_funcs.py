...
_DEFAULT_COMPACT_COEFF_COLUMNS = (
    ...
    'thetas', 'phis',
    'is_ell_perp', 'is_Ell_perp', 'is_ell_par', 'is_ell_par_rest',
...
_DEFAULT_BUCKET_FLAG_COLUMNS = {
    'ell_perp': 'is_ell_perp',
    'Ell_perp': 'is_Ell_perp',
    'ell_par': 'is_ell_par',
    'ell_par_rest': 'is_ell_par_rest',
}
...
def _build_minimal_compact_level_columns(
    ...
    * the exact per-row normalization quantities ``tau_equiv_seconds`` and
    ...
    duplicated. Instead, boolean membership flags are carried per row.
    """
    ...
    column_data: Dict[str, np.ndarray] = {
        ...
        'is_ell_perp': np.asarray(theta_mask_perp, dtype=np.float64),
        'is_Ell_perp': np.asarray(theta_mask_disp, dtype=np.float64),
        'is_ell_par': np.asarray(theta_mask_par, dtype=np.float64),
        'is_ell_par_rest': np.asarray(theta_mask_par_rest, dtype=np.float64),
...
def _finalize_compact_store_from_matrices(
    ...
    for bucket, parts in bucket_matrices.items():
        ...
        buckets_out[bucket] = {'data': data, 'n_rows': int(data.shape[0])}
    return {
        'version': 'scwf_compact_coefficients_v2',
...
def coefficient_store_to_dataframe(
    ...
    if bucket in buckets:
        ...
        if bucket not in ('ell_all', 'ell_overall'):
            ...
            if flag_col is None or flag_col not in column_order:
                raise KeyError(f'Bucket {bucket!r} is not present and no selection flag is available.')
    ...
    * if ``return_flucs`` is false, the function saves the reduced products only,
    ...
    try:
        ...
        common_meta = {
            ...
            'wavelet_meta': interval_out['meta'],
            'scale_bin_edges_di': None if scale_bin_edges_di is None else list(np.asarray(scale_bin_edges_di, dtype=float)),
            'note': 'If return_flucs/return_coefs is true, only the compact coefficient store is saved. Conditional spectra and scale-normalized higher-order moments must then be computed in a second pass from CoefficientStore. The row store is written once on ell_all, with directional membership flags and all required local scales, so rows are not duplicated across buckets.',
        }
