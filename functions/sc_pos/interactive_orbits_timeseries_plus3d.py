...
def _carrington_and_footpoints(spkid: str, start: str, stop: str, step: str, rss_rsun: float, omega_deg_per_day: float, vsw_kms: float):
    ...
    hgs = coord0.transform_to(HeliographicCarrington(obstime=coord0.obstime, observer="earth"))

    hgc_frame = HeliographicCarrington(obstime=hgs.obstime, observer="earth")
...
def build_3d_figure(
    ...
    vsw1_kms: float = 300.0,
    vsw2_kms: float | None = 700.0,
    width: int = 1800,
    height: int = 900,
    ...
    fig.add_trace(go.Scatter3d(x=[0.0], y=[0.0], z=[0.0], mode="markers+text", name="Sun", marker=dict(size=6, symbol="circle"), text=["Sun"], textposition="top center"), row=1, col=1)

    theta = np.linspace(0, 2 * np.pi, 260)
    fig.add_trace(go.Scatter3d(x=Rsun_au * np.cos(theta), y=Rsun_au * np.sin(theta), z=np.zeros_like(theta), mode="lines", line=dict(width=3), opacity=0.75, hoverinfo="skip", showlegend=False), row=1, col=2)
    fig.add_trace(go.Scatter3d(x=rss_au * np.cos(theta), y=rss_au * np.sin(theta), z=np.zeros_like(theta), mode="lines", line=dict(width=2, dash="dot"), opacity=0.40, hoverinfo="skip", showlegend=False), row=1, col=2)

    fig.add_trace(go.Scatter3d(x=earth_df["x_au"][::decimate], y=earth_df["y_au"][::decimate], z=earth_df["z_au"][::decimate], mode="lines", name="Earth (traj)", line=dict(width=3), opacity=0.55), row=1, col=1)
    ...
    for i, name in enumerate(targets):
        ...
        fig.add_trace(go.Scatter3d(x=sc_df["x_au"], y=sc_df["y_au"], z=sc_df["z_au"], mode="lines", name=name, line=dict(width=4, dash=dash, color=col)), row=1, col=1)
        fig.add_trace(go.Scatter3d(x=[float(sc_df["x_au"].iloc[-1])], y=[float(sc_df["y_au"].iloc[-1])], z=[float(sc_df["z_au"].iloc[-1])], mode="markers+text", marker=dict(size=5, color=col, symbol=sym), text=[name], textposition="top center", showlegend=False), row=1, col=1)

        for j, vsw in enumerate(vsw_list):
            t_arr, carr, fp_ss, fp_sun = _carrington_and_footpoints(spkid, start, stop, step, rss_rsun, omega_deg_per_day, vsw)
            ...
            fig.add_trace(go.Scatter3d(x=x_ss, y=y_ss, z=z_ss, mode="lines", line=dict(width=3, dash=ss_dash, color=col), opacity=alpha, showlegend=False, hoverinfo="skip"), row=1, col=2)
            fig.add_trace(go.Scatter3d(x=x_su, y=y_su, z=z_su, mode="lines", line=dict(width=5, dash=su_dash, color=col), opacity=min(0.95, alpha + 0.1), showlegend=False, hoverinfo="skip"), row=1, col=2)

    fig.update_layout(
        scene=dict(xaxis_title="x [AU]", yaxis_title="y [AU]", zaxis_title="z [AU]", aspectmode="data"),
        scene2=dict(xaxis_title="x [AU]", yaxis_title="y [AU]", zaxis_title="z [AU]", aspectmode="cube", xaxis=dict(range=[-sun_zoom_au, sun_zoom_au]), yaxis=dict(range=[-sun_zoom_au, sun_zoom_au]), zaxis=dict(range=[-sun_zoom_au, sun_zoom_au])),
    )
