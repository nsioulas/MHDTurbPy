...
def load(trange=['2018-11-5', '2018-11-6'], 
    ...
    last_version = instrument != 'wbd'
    for prb in probe:
        if instrument == 'aspoc':
            pathformat = f'c{prb}/{datatype}/asp/%Y/c{prb}_{datatype}_asp_%Y%m%d_v??.cdf'
        elif instrument in ['cis', 'dwp', 'edi', 'efw', 'whi']:
            pathformat = f'c{prb}/{datatype}/{instrument}/%Y/c{prb}_{datatype}_{instrument}_%Y%m%d_v??.cdf'
        elif instrument == 'fgm':
            if datatype == 'cp':
                pathformat = f'c{prb}/cp/%Y/c{prb}_cp_fgm_spin_%Y%m%d_v??.cdf'
            else:
                pathformat = (
                    f'c{prb}/{datatype}/{instrument}/%Y/c'
                    + prb
                    + '_'
                    + datatype
                    + '_'
